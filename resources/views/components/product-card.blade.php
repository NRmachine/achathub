<article class="shop-card p-3 d-flex flex-column">
    <a class="product-card-image" href="{{ route('products.show',$product) }}"><img src="{{ $product->image ?: asset('assets/category-accessoires.png') }}" class="product-img" alt="{{ $product->name }}" loading="lazy">@if($product->discount)<span class="badge discount">-{{ $product->discount }} %</span>@endif</a>
